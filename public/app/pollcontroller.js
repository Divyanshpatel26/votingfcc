...
app.controller('NewPollCtrl', function($scope, AuthService, $state, PollService, $http) {
    ...
    if (!$scope.isLoggedIn()) {
        ...
        $scope.poll.answers = [{
            name: 'option1',
            text: 'write here the first option'
        }, {
            name: 'option2',
            text: 'write here the second optio'
        }]
...
app.controller('PollViewController', function($scope, AuthService, $stateParams, $http, PollService) {
    $scope.new_option = "";
    ...
    $scope.vota = function() {
        if (!$scope.choice) {
            ...
            $http.patch("api/poll/vota/" + $stateParams.id, {
                "choice": $scope.choice
    ...
    $scope.view_poll_results = function() {
        $http.get("api/poll/view/" + $stateParams.id).then(function(result) {
            ...
            $scope.data = [];
            $scope.max_vote = $scope.options[0].count;
            for (var i = 0; i < $scope.options.length; i++) {
                var obj = {
                    c: []
                };
                if ($scope.max_vote < $scope.options[i].count) {
                    $scope.max_vote = $scope.options[i].count;
                }
                obj.c[0] = {
                    "v": $scope.options[i].text
                };
                obj.c[1] = {
                    "v": $scope.options[i].count
                };

                $scope.myChartObject.data.rows.push(obj);
            }
             $scope.myChartObject.options =  {vAxis: {minValue:0,maxValue: $scope.max_vote,gridlines:$scope.max_vote,  format:'0',gridlines: { count: $scope.max_vote+1}}};
    ...
    $scope.confirm_new_option = function() {
        ...
        var obj_add = {
            name: 'option' + (n + 1),
            text: $scope.new_option
        };
        $scope.options.push(obj_add);
        $http.patch("/api/poll/addoption/" + $stateParams.id, {
            "option": obj_add
        }).then(function(result) {
...
app.controller('MyCtrl', function($scope, AuthService, $state, $http, PollService) {
    $scope.isLoggedIn = AuthService.isAuthenticated;
    if (!$scope.isLoggedIn()) {
        $state.go('app.login');
    }
